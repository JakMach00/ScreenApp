...
export class ScreenRecorder {
  ...
  async start(options: RecordOptions, onInterrupted?: () => void): Promise<void> {
    if (this.recorder) throw new Error('Nagrywanie juz trwa.');
    ...
    if (!ctx) throw new Error('Brak kontekstu 2D dla nagrywania.');
  ...
  async stop(): Promise<RecordResult> {
    ...
    if (!recorder) throw new Error('Nagrywanie nie jest aktywne.');
