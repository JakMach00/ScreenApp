...
export async function buildPdf(shots: Shot[], options: PdfOptions): Promise<Uint8Array> {
  ...
  if (pages.length === 0) throw new Error('Brak materialu do zapisania w PDF.');
  ...
  for (let i = 0; i < pages.length; i += 1) {
    ...
    if (shot.kind === 'video') {
      ...
      doc.setTextColor(40);
      doc.text(
        `Nagranie ekranu, dlugosc ${formatDuration(shot.durationMs)}. Plik wideo zapisany obok tego PDF.`,
        MARGIN,
